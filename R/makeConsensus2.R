...
#'
#' @aliases makeConsensus2
#'  
#' @param x a matrix or \code{\link{ClusterExperiment}} object.

#' @param algorithm choosing wich algorithm
...
#'  clusterTypes equal to \code{makeConsensus2} and the \code{percentageShared}
...
#' clCommon100 <- makeConsensus2(clMat, proportion=1, minSize=10)
#'
#' #require 70% agreement based on clustering of overlap
#' clCommon70 <- makeConsensus2(clMat, proportion=0.7, minSize=10)
#'
#' oldpar <- par()
...
#' clCommon <- makeConsensus2(cl, whichClusters="workflow", proportion=0.7,
#' minSize=10)
#' plotClusters(clCommon)
#' par(oldpar)
#'
#' @rdname makeConsensus2
#' @export
setMethod(
  f = "makeConsensus2",
                        ...
                        propUnassigned=.5, minSize=5,...) 
  {
    ...
    #  if(typeAlg!="01") {
    #    stop("makeConsensus2 is only implemented for '01' type clustering functions (see ?ClusterFunction)")
    #  }
    ...
    #clUnassigned <- cl #questo deve essere membership
    #clUnassigned[whUnassigned] <- -1
    
    return(list(clustering=clustering$membership, percentageShared=NULL,
                noUnassignedCorrection=clustering$membership)) #memmbership sia su clustering che su unassigned
  }
)
...
#'   default it is equal to "makeConsensus2", to indicate that this clustering is
#'   the result of a call to makeConsensus2. However, a more informative label can
#'   be set (see vignette).
setMethod(
  f = "makeConsensus2",
  signature = signature(x = "ClusterExperiment"),
  definition = function(x, whichClusters, eraseOld=FALSE,clusterLabel="makeConsensus2",...){
    ...
    outlist <- makeConsensus2(clusterMat, ...)
    newObj <- ClusterExperiment(x, outlist$clustering,
                                transformation=transformation(x),
                                clusterTypes="makeConsensus2",checkTransformAndAssay=FALSE)
    ...
    x<-.updateCurrentWorkflow(x,eraseOld,newTypeToAdd="makeConsensus2",newLabelToAdd=clusterLabel)
